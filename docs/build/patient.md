...
| 10 | Flutter: nurse screens, then the patient's own-stay screens | **Done, then reversed 2026-09-21.** `mobile-ui/` is a real Flutter app with `android/` and `ios/`, the generated `api_client`, and the patient feature under `lib/features/patient/`: `patient_shell.dart` (the tab shell), `home_screen.dart`, `my_stay_screen.dart` (current admission status, bill summary, and — since step 16 — the care advisory "How are you feeling?" card), `past_visits_screen.dart`, `appointments_screen.dart` + `book_appointment_sheet.dart`, `my_details_screen.dart`, `profile_screen.dart`, `my_reports_screen.dart` (lab reports), `bill_sheet.dart`, `claim_record_screen.dart` (linking an unclaimed record by patient code). Local notifications on status change is the device feature. **The nurse screens named here were removed the same day** — see the row 12/13/16 notes below and `patient-management-plan.md` §10 |
| 11 | **`PatientMedicalProfile` entity + configuration + migration + seed** | **Done 2026-09-18.** One table, one row per patient, `UNIQUE(patient_id)`, `GET`/`PUT /patients/{id}/medical-profile` behind `MedicalProfileReader` / `MedicalProfileAuthor`. **The editor is React-only** — `web-ui/src/components/MedicalProfilePanel.tsx`, opened from `PatientsPage`. The Flutter screen off the nurse worklist mentioned here was removed 2026-09-21; mobile has no staff screens. Seeded in `docs/seed/005_patient_medical_profiles.sql` — two patients are deliberately left without a profile so the agent's empty-profile path can be demonstrated |
| 12 | ~~The bed agent~~ | **Built 2026-09-20, reworked 2026-09-21, removed 2026-09-22.** Lived in `api/Agents/Patient/` behind `POST /api/bed-suggestions` and `GET /api/bed-workflows/{workflowId}`. Taken out because a second, purpose-built agent (Patient Care Advisory) covered the assignment's agent requirement better than an agent for a task a human already does in one manual step. No trace left in the contract, the database, or either client — removal merged to `main` in PR #89 |
...
| 17 | **Review fixes** | **Done 2026-09-25**, branch `fix/patient-review-fixes`. Settling a bill rebuilds its generated lines (a bill raised on day 1 and settled on day 5 used to charge one night). Admission bills only while on the ward (`cl_pat_044`), appointment bills only on a finished booking (`cl_pat_045`), checklist only while on the ward (`cl_pat_046`). Ward fit H7 (`BedPlacementRules.FitsWard`, `cl_pat_047`, duty manager may overrule). `maternity` refused for a male patient (`cl_pat_048`). Appointments: one **Check in** panel whose list is the walk-in list plus **No bed needed** (the latter completes the booking and bills it there); the ICU-only-for-duty-manager rule at check-in retired (`cl_pat_011`). Care agent: 3 reports a minute (`cl_pat_050`), red flag returned at once and shown in Flutter, text normalised before the keyword screen, Approve/Reject/Redraft shut while a run is going (`cl_pat_049`), card shown only while admitted. Dead no-bed code removed: `RequiresBed`, `requires_bed`, `POST /admissions/{id}/complete`, `awaiting_bed → admitted`, `cl_pat_020`/`021`. Expected visits list uses the Sri Lanka calendar day. A corrected bed on a lapsed hold gets a fresh hold. **Identity:** pre-register refuses a NIC already on a hospital record (`cl_pat_051`, the app offers "Use my patient code") instead of linking it; `category_set_by_staff_id` is always the signed-in user and left the request bodies (`cl_pat_008` retired) The medical profile is three fields now: `recent_situation` was folded into `current_symptoms` (migration `Patient_RemoveMedicalProfileRecentSituation` moves any existing text across) |
| 18 | **First-time patients and walk-ins on Appointments** | **Done 2026-09-25**, branch `feat/patient-appointments-new-patients`. "Expected visits" renamed **Appointments** (on-screen text only; route, API and table were already `appointments`). Book a visit → search → **Register a new patient** (name, mobile, gender, optional NIC — plain `POST /patients`). New `POST /appointments/walk-in` (`createWalkInAppointment`): now, already confirmed. Check in to a ward asks for walk-in intake's full details first if the record is short; emergency and No bed needed do not. Plan: `patient-management-plan.md`, after the Check in table |
| 19 | **End-to-end test fixes** | **Done 2026-09-26**, branch `fix/patient-e2e-defects`. From the 2026-09-25 walk-through. Name, gender and NIC locked to the administrator once a NIC is recorded (`PatientIdentityRules`, `cl_pat_052`); gender change refused against the current ward's policy. Unanswered care queries closed on discharge. Confirmed visits cancellable in the app (`can_cancel`). CR5 accepts "cannot". Web: waiting-queries count on the dashboard tile, Approve needs a reply, duty manager cancels an admission, settle asks first, gender starts unchosen. Mobile: care replies reload on refresh/tab, whole conversation shown, 2-minute countdown after the 3-per-minute limit. Follow-ups the same day, branch `fix/patient-e2e-followups`: the care agent saves its steps as they finish so the reviewer's progress list keeps up (D14); only a per-day Gemini 429 counts as quota exhausted, a per-minute one is retried (D22); demo contact number and email on the app's call-the-desk help (D15). Open: D1 (shared `transport.ts`), D25. D16 kept by decision |
...
**Steps 14–16 landed 2026-09-21 and are merged into `main`** (PR #87, tuned further in PR #88).
Not yet clicked through in a browser — built and verified by 6 new integration tests through a real Postgres
