...
| M3 Equipment | `wardId` filter on `GET /admissions` | **M4** | **Not a stub and nothing is blocked** — the laboratory ward list is built and works. It reads `IAdmissionService.ListAsync` through a port and then matches on **`ward_name`**, because `AdmissionSummary` publishes a name and not an id, filtering in Equipment after the read. Honest at a few hundred beds, wrong at ten thousand. M4's own row above already calls this filter unblocked; when it lands, `InHospitalPatientDirectory` becomes one delegating call. Raised 2026-09-13, `integration_of_functions.md` §11.15 |
