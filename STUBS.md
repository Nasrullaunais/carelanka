...
| **M4 Patient** | **`AgentWorkflow` — the record of what an agent did** | **Common (the group)** | **STUBBED 2026-09-18, by M4, locally and not pushed. This row is the whole reason the file exists — read it before the group builds the real one.** ADR 3 settled the design on 2026-09-07 and nobody built it, so the bed agent had nothing to persist a run in and §9.1 scores persistence. What exists now is a **real table, not an in-memory fake**: `api/Data/Entities/Common/AgentWorkflow.cs`, its configuration, and the `Patient_AddBedAgent` migration, copied field for field from `entity_diagram.md` and `common-spec.yaml` so the group's version drops in on top. Three things the group has to rule on, all flagged in code: **(1)** `Objective` is stored as the `WorkflowObjective` enum with `HasConversion<string>` (ADR 5), where the diagram says plain `string` — same column, typed differently. **(2)** the bed agent's runs set `EntityType = "BedSuggestion"`, not `"Admission"`, because a run started from an NIC has no admission yet and `EntityId` is non-null; the admission is one join away. **(3)** none of `/api/workflows*` is built — this is the table only, read through the Patient-specific `GET /api/bed-workflows/{id}`. **Delete all of it when common lands**; `git grep AgentWorkflow` finds every file |
| **M4 Patient** | **`AgentProposedChange`** | **Common (the group)** | **Deliberately NOT stubbed, and that is the right answer.** The bed agent holds no write tool at all (`patient-management-plan.md` §8.6b), so it produces zero proposal rows and there was nothing to copy honestly. Patient Management's own answer lives in `bed_suggestions` / `bed_suggestion_candidates`, which are **M4's tables, not a copy of the group's** — a suggestion is a list of beds a human may press a button on, never a domain write waiting to be applied. The care advisory agent may need the real thing; the bed agent does not |
...
| M3 Equipment | `wardId` filter on `GET /admissions` | **M4** | **Not a stub and nothing is blocked** — `GET /ward-patients` is built and works, behind both the laboratory screen and the equipment assign dialog. It reads `IAdmissionService.ListAsync` through a port and then matches on **`ward_name`**, because `AdmissionSummary` publishes a name and not an id, filtering in Equipment after the read. Honest at a few hundred beds, wrong at ten thousand. M4's own row above already calls this filter unblocked; when it lands, `WardPatientService` becomes one delegating call. Raised 2026-09-13, `integration_of_functions.md` §11.15 |
...
| ~~all four~~ | ~~Agent workflow tables~~ | ~~group~~ | **Not a stub — DECIDED 2026-09-07.** Common, built once. Contract: `specs/common-spec.yaml` (`GET /workflows`, `GET /workflows/{workflowId}`, the approve/reject/revise gate). Reasoning: `docs/ADR.md` ADR 3. **Still unbuilt as of 2026-09-18** — M4 has a local `AgentWorkflow` table standing in; see M4's row above |
